...
test('join2 is added to [].__proto__', () => {
  expect([].join2).toBeInstanceOf(Function);
});

test(`join2 doesn't call default join`, () => {
  expect([].join2.toString().includes('.join(')).toBe(false);
});
...
test(`Should work when array has null and undefined`, () => {
  expect([false, null, 1, undefined, 0, NaN, ''].join2(',')).toBe(
    'false,,1,,0,NaN,'
  );
});

test(`Should work when array has undefined as last element`, () => {
  expect([1, 2, 3, 4, undefined].join2(',')).toBe('1,2,3,4,');
});

test(`Should work when array has null as a first element`, () => {
  expect([null, null, 1, undefined, 0, NaN, ''].join2(',')).toBe(',,1,,0,NaN,');
});

test(`Should work when array contains a string 'null'`, () => {
  expect([null, null, 1, undefined, 0, 'null', NaN, ''].join2(',')).toBe(
    ',,1,,0,null,NaN,'
  );
});

test(`Should work with object as a separator`, () => {
  expect(source.join2({})).toBe(
...
test(`Should not mutate original array`, () => {
  ...
  expect(
    copyOfTrickyArray.every((element, index) => trickyArray[index] === element)
  );
});
